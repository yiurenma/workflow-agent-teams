...
- [x] **部署 — 重写 `deploy`：点击后自动调 online API；执行名 vs 源应用；双块请求体；JSON 工作流 + AI Generate** *(label: `TODO-deploy-rewrite-online-api-workflow-json`)* — ✅ **DONE** — 2026-04-29. Rewrote `DeployModal.tsx`: removed 3-step target-server flow (createApplicationName → updateSettings → saveWorkflow) and all form fields for Deploy URL, Username, Password, Environment. New single-field form: **Execution Application Name** (pre-fills from source app). On Deploy: assembles `{ messageInformation: { sourceApp: currentSettings, workflow: currentWorkflow } }` from the already-fetched props and calls `POST /api/proxy/online/workflow?applicationName={userInput}`. No CORS proxy or credential handling needed. Commit: `workflow-ui@472260d`. Branch: `claude/happy-franklin-utx2Y`. **Label:** `TODO-deploy-rewrite-online-api-workflow-json`.

- [x] **online-api — POST 可选 SSE：每步完成后将 DB 中已保存的 Runtime（事件）数据推送给前端** *(label: `TODO-online-api-post-optional-sse-runtime-per-step`)* — ✅ **DONE** — 2026-04-29. Added opt-in SSE path to `POST /api/workflow`. **Opt-in header: `X-Stream-Response: true`.** When present, Spring routes to a new `@PostMapping(headers="X-Stream-Response=true", produces=TEXT_EVENT_STREAM_VALUE)` mapping that returns `SseEmitter` (120 s timeout) and runs the dispatch pipeline in a JDK-21 virtual thread. Emits `event:step_complete` with the saved `WorkflowRecord` as JSON after enrichment-phase DB save and after every dispatch-step DB save. Emits `event:error` on failure. Without the header the existing `ResponseEntity<Void>` mapping is fully unchanged — zero impact on default clients. `WorkflowDispatchService` refactored to accept optional `Consumer<WorkflowRecord> stepEventEmitter` via new overloads; all existing call paths pass `null`. Commits: `workflow-online-api@0d25944`. Branch: `claude/sharp-hypatia-utx2Y`. **Label:** `TODO-online-api-post-optional-sse-runtime-per-step`.

- [x] **画布 — Test：请求体执行后以 SSE 逐步展示每步 API 响应（JSON）+ 大体量 UI/性能** *(label: `TODO-canvas-test-sse-per-step-response-ui-performance`)* — ✅ **DONE** — 2026-04-29. Canvas Run modal updated with optional SSE streaming. **New "Stream per-step responses (SSE)" checkbox** in the Run modal; when checked the call goes via `onlineApi.postWorkflowStream()` which adds `X-Stream-Response: true` header and reads the response as a `ReadableStream`, parsing SSE `event/data` blocks manually. Each `step_complete` event appends a collapsible card to the step list: only the first step is expanded by default; JSON is formatted lazily on expand (via `JSON.stringify(JSON.parse(raw), null, 2)`) to avoid main-thread cost on large payloads. Cancel Stream button aborts the fetch via `AbortController`. Non-SSE mode preserves existing single-response behaviour exactly. `online.ts` extended with `postWorkflowStream()` and `StepEvent` type. Commit: `workflow-ui@472260d`. Branch: `claude/happy-franklin-utx2Y`. **Label:** `TODO-canvas-test-sse-per-step-response-ui-performance`.
