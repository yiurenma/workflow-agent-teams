...
- [ ] **UI — 输入框与按钮高度一致；主/危险按钮文字必须可见** *(label: `TODO-ui-form-control-heights-and-button-label-contrast`)* — **Open.** 全应用表单里输入框与按钮竖向高度不一致，视觉杂乱；红色「删除」等按钮背景为红但文案不显示（可能与 `color` / 对比度 / 继承样式有关），蓝色主按钮亦需排查。**Umbrella 记录:** `yiurenma/workflow` 仓库 `docs/TODO.md` 小节「UI — Consistent control heights…」。**Scope:** `workflow-ui` 全局表单与按钮组件。**Label:** `TODO-ui-form-control-heights-and-button-label-contrast`.

- [ ] **部署 — Deploy 第一步因 Deploy URL 跨域 (CORS) 失败；需经代理 API** *(label: `TODO-deploy-step1-cors-proxy-for-target-base-url`)* — **Open.** 用户在 Deploy 表单填写目标 **Deploy URL** 后，浏览器对第一步 **CreateApplicationName** 等请求报跨域（用户口语「交叉引用」错误）；应在 **proxy / 同源后端** 代发请求，而非由浏览器直连任意目标主机。**Umbrella 记录:** `yiurenma/workflow` `docs/TODO.md` 小节「Deploy — Step 1 fails… CORS… proxy API」。**Scope:** `workflow-ui` Deploy 流程 + `workflow-operation-api`（或独立代理）按 Arch 定稿。**Label:** `TODO-deploy-step1-cors-proxy-for-target-base-url`.

- [ ] **校验 — 放宽 `uiMapList` 与 `pluginList` 对 IFELSE 分支边 id 的检查** *(label: `TODO-validation-relax-uiMapList-ifelse-edges-vs-pluginList`)* — **Open.** UAT 上出现大量「`uiMapList[n].source/target`：`IFELSE_1`… 不存在于 `pluginList`」导致整页 **Validation failed**；测试/分支流中不应强制每条 IFELSE 边端点都在 `pluginList` 中有 1:1 条目。**Umbrella 记录:** `yiurenma/workflow` `docs/TODO.md` 小节「Validation — Relax uiMapList…」。**Scope:** `workflow-ui` 校验与（如适用）mapper。**Label:** `TODO-validation-relax-uiMapList-ifelse-edges-vs-pluginList`.
