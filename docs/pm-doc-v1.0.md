...
|-----------|------------|
| `workflow-ui` | Visual workflow editor — drag-drop canvas, application list, test runner |
...
> As an integration engineer, I want to replicate a workflow definition to a new application so I avoid rebuilding identical configurations.

**Acceptance criteria**:
- AC-10-1: An "Auto Copy" action accepts `fromApplicationName` and `toApplicationName`.
...
- AC-10-4: The copy creates a new `WorkflowEntitySetting` for the target with all rules, types, and step order duplicated.
...
> As an operations engineer, I want to see the change history of an application's workflow configuration so I can audit what changed and when.

**Acceptance criteria**:
- AC-11-1: `GET /api/workflow/entity-setting/history?applicationName=APP` returns a list of revisions in reverse chronological order.
- AC-11-2: Each revision includes: revision number, timestamp, change type (ADD / MOD / DEL), and the snapshot of `WorkflowEntitySetting` at that revision.
- AC-11-3: History is available regardless of whether the application still has execution records.

---

### 3.5 Workflow Execution (Online API)
...
| **Async execution** | Enrichment and dispatch run asynchronously after the `200` response is returned |
| **Audit trail** | Every change to `WorkflowEntitySetting` must be tracked via Envers revision history |
...
- Real-time execution status push (WebSocket / SSE) — current model is fire-and-forget with poll
...
| # | Question | Impact |
|---|----------|--------|
| OQ-1 | Should the UI surface `WORKFLOW_RECORD` execution history (a "runs" tab per application)? | Scope of Online API read endpoints; new UI page |
| OQ-2 | Is FUNCTION_V2 the only FUNCTION variant to support going forward, or should FUNCTION (v1) remain configurable? | Simplification of drawer forms and backend type handling |
| OQ-3 | Should "enabled/disabled" toggle on `WorkflowEntitySetting` be actionable from the UI list page (toggle in-place)? | New UI interaction + PATCH endpoint on Operation API |
| OQ-4 | Are there plans to add new channel types beyond SMS/Email/Push? | Enum extensibility in `WorkflowRecord` status columns |
| OQ-5 | Should retry configuration (error codes, max attempts) be editable per step in the UI, or remain backend-only config? | New drawer fields + Operation API contract change |
